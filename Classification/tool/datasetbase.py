...
class DataSet(object):
    ...
    def format(self, batch):
        ...
        for i in range(0, len(t)-1, 2):
            ...
            max_len = max(txt_len)
            txt = self.single_sent_pad(txt, max_len=max_len)
            # 
    ...
    def single_sent_pad(self, sents, max_len=None):
        """
        Fun:
            padding the input sents to the same length

        Args:
            sents, sentence list, each element is one sentence (word separated by a space).

        Return:
            tuple(sent_matrix, lenghts), 
            sent_matrix, a numpy with shape (sample_num, max_length)
            lengths, a length list of sents.
        """
        ...
        for ind, sent in enumerate(sents):
            words = sent # 
            if self.sos_id is not None:
                words = [self.sos_id] + words 
            if self.eos_id is not None:
                words.append(self.eos_id)
            sent_len = len(words)
            X[ind, :sent_len] = words
        # write a tuple class to save the result
        return X
